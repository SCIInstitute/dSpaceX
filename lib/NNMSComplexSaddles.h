...
class NNMSComplex{
  ...
  private:
    struct Merger{
       Merger(int e1, int e2, int s1, in s2){
         ext.first = e1;
         ext.second = e2;
         saddle.first = s1;
         saddle.second = s2;
    ...
    typedef std::map< std::pair<int, int>, std::pair<int, int> > map_pi_pi;
    typedef map_pi_pi::iterator map_pi_pi_it;
    ...
    typedef typename std::map< TPrecision, Merger, std::less<TPrecision>  > map_f_m;
    typedef typename map_f_m::iterator map_f_m_it;
...
private:
    ...
    void runMS(bool smooth){
          ...
          for(unsigned int k=1; k < KNN.M(); k++){
            ...
            if(e1 != e2){
              ...
              if(it!=pinv.end()){
                ...
                if(pers < tmp){
                  (*it).second = pers;
                  pinvSaddles[p] = std::pair<int, int>(KNN(k, i) , i);
                }
              }
              else{
                pinv[p] = pers;
      ...
      for(map_pi_f_it it = pinv.begin(); it != pinv.end(); ++it){
        Merger m;
        m.ext = (*it).first;
        m.saddle = pinvSaddles[(*it).second];
        persistence[(*it).second] = m;
        //persistence[(*it).second] = (*it).first;
      }


      //compute final persistencies - recursively merge smallest persistence
      //extrema and update remaining persitencies depending on the merge
      for(unsigned int i=0; i<merge.N(); i++){
        merge(i) = i;
      } 
      ...
      while(!persistence.empty()){
        ...
        Merger cur = (*it).second.
        std::pair<int, int> &p = cur.ext;
        //std::pair<int, int> p = (*it).second;
        ...
        //find new merging pair, based on possible previous merges
