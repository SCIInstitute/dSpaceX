...
wv_makeStripes(wvGPrim *gp, int bias)
{
  ...
  if  (gp->nVerts <= maxLen) {
    ...
    if ((gp->indices  == NULL) && (gp->lIndices == NULL) && 
      ...
      if ((gp->gtype == WV_POINT) || (gp->gtype == WV_POINT2D)){
      ...
      } else if ((gp->gtype == WV_LINE) || (gp->gtype == WV_LINE2D) ||
                 (gp->lIndices == NULL)) {
            ...
            int nItems, wvData *items)
{
  ...
  if ((gtype != WV_POINT) && (gtype != WV_LINE) && (gtype != WV_TRIANGLE)) {
    if ((gtype != WV_POINT2D) && (gtype != WV_LINE2D) &&
        (gtype != WV_TRIANGLE2D)) return -5;
    if (gtype == WV_TRIANGLE2D)
      for (i = 0; i < nItems; i++)
...
wv_writeGPrim(wvGPrim *gp, void *wsi, unsigned char *buf, int *iBuf)
{
  ...
  for (i = 0; i < gp->nStripe; i++) {
    ...
    npack += nd*4*gp->stripes[i].nsVerts;
...
wv_sendGPrim(void *wsi, wvContext *cntxt, unsigned char *buf, int flag)
{
  ...
  /* put out the key if there or has changed */
  if ((cntxt->titleLen == -1) || ((flag == -1) && (cntxt->nColor != 0)) ||
    ...
    if ((cntxt->titleLen == -1) || (cntxt->nColor == 0) ||
      ...
      iBuf += 4;
      for (i = 0; i < 3*cntxt->nColor; i++, iBuf += 4)
