...
class LegacyCrystalImpl : public Crystal{
 public:
  LegacyCrystalImpl(unsigned int minIndex, unsigned int maxIndex, std::vector<dspacex::ValueIndexPair> samples) :
  ...
  virtual std::vector<dspacex::ValueIndexPair>& getAllSamples() { return m_samples; }
 private:
  unsigned int m_minIndex;
  unsigned int m_maxIndex;
  std::vector<dspacex::ValueIndexPair> m_samples;
};
