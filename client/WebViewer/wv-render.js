...
"          color           = color*wColor + vec4(conColor,1.0)*(1.0-wColor);",
"          v_Color         = color*dot + color*wAmbient;",
"          v_bColor        = v_Color;",
"          // are we coloring the backface?",
"          if (bColor != 0.0) {",
"              color       = vec4(bacColor,1.0);",
...
wv["Start"] = function()
{
  ...
  //
  // get the 2D context (if available)
  var gl2d = undefined;
  if (wv.canvas2D != undefined) {
    gl2d = wv.initWebGL(wv.canvas2D);
    if (!gl2d) return;
    
    // define the shaders -- finish up the 2D WebGL setup
    wv.InitCanvas2D(gl2d);
  }
