...
void DisplayTubes<TPrecision>::display(void){
  ...
  if (drawOverlay) {
    ...
    ssv << (data->getWidth(state->currentLevel)[state->selectedCell](state->selectedPoint)-0.03) * data->getZMax(state->currentLevel)/0.3;
    ...
    std::vector<Precision> color =
      data->getColorMap().getColor(data->getExtremaMinValue(state->currentLevel));
    glColor3f(color[0], color[1], color[2]);
    glVertex2f(0, 0);
    glVertex2f(0.4, 0);

    color = data->getColorMap().getColor((data->getExtremaMinValue(state->currentLevel) + data->getExtremaMaxValue(state->currentLevel))/2.f);
    glColor3f(color[0], color[1], color[2]);
    glVertex2f(0, 1);
    glVertex2f(0.4, 1);      
    
    color = data->getColorMap().getColor(data->getExtremaMaxValue(state->currentLevel));
    glColor3f(color[0], color[1], color[2]);
    glVertex2f(0, 2);
    glVertex2f(0.4, 2);
    glEnd(); 
...
void DisplayTubes<TPrecision>::renderWidths() {
  ...
  for (unsigned int i = 0; i < data->getEdges(state->currentLevel).N(); i++) {
    ...
    if (selectedCTubes(i)) {
      ...
      glutSolidSphere(scale * data->getExtremaWidths(state->currentLevel)(i1), 50, 50);
      glPopMatrix();
      ...
      glutSolidSphere(scale * data->getExtremaWidths(state->currentLevel)(i2), 50, 50);
      glPopMatrix();
  ...
  for (unsigned int i = 0; i < data->getEdges(state->currentLevel).N(); i++) {
    ...
    if (selectedCTubes(i)) {
      ...
      for (unsigned int k = 0; k < layout[i].N(); k++) {
        std::vector<Precision> color = data->getDColorMap().getColor(data->getDensity(state->currentLevel)[i](k));
        colors[k+1][0] = color[0];
        colors[k+1][1] = color[1];
        colors[k+1][2] = color[2];
        colors[k+1][3] = 1;
      ...
      glTranslatef(
          ...
          extremaLayout(1, i1), 
          extremaNormalized(i1)); 
      glutSolidSphere(scale *data->getExtremaWidths(state->currentLevel)(i1) + scale*0.025/zoom, 50, 50);
      glPopMatrix();
      ...
      glTranslatef(
          ...
          extremaLayout(1, i2), 
          extremaNormalized(i2)); 
      glutSolidSphere(scale *data->getExtremaWidths(state->currentLevel)(i2)+ scale*0.025/zoom, 50, 50);
      glPopMatrix();
  ...
  for (unsigned int i = 0; i < extremaLayout.N(); i++) {
    if (selectedPeaks(i) == true) {
      ...
      glTranslatef(
          ...
          extremaLayout(1, i), 
          extremaNormalized(i)); 
      glutSolidSphere(scale *data->getExtremaWidths(state->currentLevel)(i), 50, 50);
      glPopMatrix();
      ...
      glTranslatef(
          ...
          extremaLayout(1, i), 
          extremaNormalized(i)); 
      glutSolidSphere(scale *data->getExtremaWidths(state->currentLevel)(i) + scale*0.015/zoom, 50, 50);
      glPopMatrix();
