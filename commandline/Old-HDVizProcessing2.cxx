...
DenseVector<int> crystalIDs;
DenseMatrix<int> crystals;
DenseVector<Precision> persistence;
...
int main(int argc, char **argv){
  ...
  try{
    ...
    if(randArg.getValue()){
       ...
       double a = 0.0001 *( Linalg<Precision>::Max(yall) - Linalg<Precision>::Min(yall));
    ...
    NNMSComplex<Precision> msComplex(Xall, yall, knn);
    ...
    for(unsigned int nP = start; nP < persistence.N(); nP++){
      ...
      DenseMatrix<Precision> ScrystalIDs[crystals.N()];
      DenseMatrix<Precision> XcrystalIDs[crystals.N()];
      DenseMatrix<Precision> XpcrystalIDs[crystals.N()];
      DenseMatrix<Precision> ycrystalIDs[crystals.N()];
      DenseVector<Precision> eWidths(exts.size());
      Linalg<Precision>::Zero(eWidths);

      std::vector<unsigned int> Xi[crystals.N()];
      std::vector<unsigned int> Xiorig[crystals.N()];
      std::vector<Precision> yci[crystals.N()];
	...
	GaussianKernel<Precision> kernel(sigma, 1);
        KernelRegression<Precision> kr(X, y, kernel);
...
*/
	...
	Linalg<Precision>::Set(gStmp, 0);
        ...
        for(int k=0; k < nSamples; k++){
          ...
          Zp(0, k) = z(0);
          pdist(k) = kr.evaluate(z, tmp, sdev);
